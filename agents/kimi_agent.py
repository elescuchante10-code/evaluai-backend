"""
KimiAgent - Agente para API de Kimi (Moonshot AI)
Compatible con suscripción Allegretto y modelo K2.5
"""
import os
import json
...
@dataclass
class KimiAgentOptions(AgentOptions):
    """Opciones específicas para Kimi Agent"""
    model: str = "kimi-k2-5"  # Modelo flagship con suscripción Allegretto
...
class KimiAgent(Agent):
    """
    Agente de evaluación académica usando Kimi API (Moonshot AI)
    Optimizado para suscripción Allegretto con modelo K2.5
    """
...
FORMATO DE RESPUESTA REQUERIDO:
{
    ...
    def __init__(self, options: KimiAgentOptions):
        super().__init__(options)
        
        # Configurar cliente OpenAI compatible con Kimi (Moonshot)
        self.client = openai.AsyncOpenAI(
            base_url=options.base_url or os.getenv("KIMI_BASE_URL", "https://api.moonshot.cn/v1"),
            api_key=options.api_key or os.getenv("KIMI_API_KEY")
        )
        ...
        # Tracking de costos (Kimi tiene precios diferentes)
        self.total_tokens_input = 0
        self.total_tokens_output = 0
        
        # Configurar system prompt
        self.system_prompt = self.DEFAULT_SYSTEM_PROMPT
        Logger.info(f"KimiAgent inicializado: {self.name} usando modelo {self.model}")
    
    def is_streaming_enabled(self) -> bool:
        """Kimi soporta streaming"""
        return True
    ...
    async def process_request(
        ...
        try:
            ...
            Logger.error(f"Error en KimiAgent: {str(e)}")
    ...
    def get_cost_estimate(self, input_tokens: int, output_tokens: int) -> Dict:
        """Calcula costo estimado en USD y COP según precios de Kimi"""
        # Precios aproximados de Kimi (pueden variar según tu plan Allegretto)
        # Generalmente más económico que OpenAI
        cost_per_1k_input = float(os.getenv("KIMI_COST_PER_1K_INPUT", 0.001))
        cost_per_1k_output = float(os.getenv("KIMI_COST_PER_1K_OUTPUT", 0.003))
